...
GameOverDialog(
  document.body,
  'Start Game',
  localStorage.getItem('score_' + localStorage.getItem('LoginId')),
  'game.startGame',
  'Start',
  'Start'
);
    ...
    this.idInterval = setInterval(() => {
      ...
      for (let j = 0; arrBullet.length > j; ++j) {
        for (let i = 0; arrEnemy.length > i; ++i) {
          if (
            collisionDetection(arrBullet[j].bulletElement, arrEnemy[i].enemy)
          ) {
...
class Game {
  ...
  createEnemyRow() {
    ...
    const startingPosition = (this.battlefield.offsetWidth - totalRowWidth) / 2;
    // Create a new row
    const enemyRow = [];
